...
const API_URL = "http://localhost:5000/api/data3";
...
const Data3 = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`${API_URL}/getdata3`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        ...
        await axios.put(`${API_URL}/updatedata3/${editId}`, formData);
        setEditId(null);
      } else {
        // Create new entry
        await axios.post(`${API_URL}/postdata3`, formData);
      }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${API_URL}/deletedata3/${id}`);
      fetchData(); // Refresh list
    } catch (err) {
